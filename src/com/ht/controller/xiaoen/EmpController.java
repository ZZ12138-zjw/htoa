...
public class EmpController {
    ...
    @Autowired
    private ICheckInsert checkInsert;

    @Autowired
    private ICheckIndex checkIndex;
    ...
    @ResponseBody
    public Map checkList(int empId) {
        Map map = new HashMap();
        map.put("code", 0);
        map.put("msg", "");
    ...
    public String docDelete(DocumentVo documentVo) {
        documentEmpService.delete(documentVo);
        return "success";
    }


    @RequestMapping("/to_checkInsertAdd")
    public String toCheckInsertAdd(String empId, Map map) {
        map.put("allCheckInsertList", checkInsert.selectAll());
        map.put("empId", empId);
        return "checklnsert_add";
    }

    @RequestMapping("/checkInsertAdd")
    @ResponseBody
    public String heckInsertAdd(CheckInsertVo checkInsertVo, HttpSession session) {
        EmpVo empVo = (EmpVo) session.getAttribute("empVo");

        List<CheckIndexVo> list = emp.findCheckFraction(checkInsertVo.getCheckContent());
        for (CheckIndexVo checkIndexVo : list) {
            checkInsertVo.setCheckScore(checkIndexVo.getCheckScore());
        }
        checkInsertVo.setEmpName(empVo.getEmpName());
        checkInsertVo.setEmpID(empVo.getEmpId());
        checkInsertVo.setInputEmp(empVo.getEmpName());

        checkInsert.addCheckInsert(checkInsertVo);
        return "success";
    }

    @RequestMapping("/to_checkInsertUpdate")
    public String to_checkInsertUpdate(String id, Map map) {
        map.put("allCheckInsertList", checkInsert.selectAll());
        map.put("ckVo", checkInsert.selInsertByID(Integer.parseInt(id)));
        return "checklnsert_update";
    }


    @RequestMapping("/checkInsertUpdate")
    @ResponseBody
    public String checkInsertUpdate(CheckInsertVo checkInsertVo,HttpSession session) {

        EmpVo empVo = (EmpVo) session.getAttribute("empVo");

        System.out.println(checkInsertVo.toString());
        List<CheckIndexVo> list = emp.findCheckFraction(checkInsertVo.getCheckContent());
        for (CheckIndexVo checkIndexVo : list) {
            checkInsertVo.setCheckScore(checkIndexVo.getCheckScore());
        }
        ...
        checkInsertVo.setEmpID(empVo.getEmpId());
        checkInsertVo.setInputEmp(empVo.getEmpName());
        checkInsert.editCheckInsert(checkInsertVo);

        return "success";
    }

    @RequestMapping("/checkInsertDalete")
    @ResponseBody
    public String checkInsertUpdate(String id) {
        checkInsert.delCheckInsert(id);
